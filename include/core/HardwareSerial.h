...
{
public:
	HardwareSerial(): sercom((SERCOM_USART *)0xE01000000) {}
	
	// 綁定SERCOM物件，若其他ISercomIRQ物件後來綁定了同個SERCOM物件，就需要再呼叫此方法.
	// 設定IO腳位多工，若IO腳位被挪作他用就需要再呼叫此方法.
	void configIO(SERCOM *, PinTuple RX, PinTuple TX);
	
	// baud, RX pinout, TX pinout, config
    void begin(uint32_t, SercomRXPad, SercomUartTXPad, uint16_t config =SERIAL_8N1);
	void end();
