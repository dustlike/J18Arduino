...
void HardwareSerial::configIO(SERCOM *scm, PinTuple RX, PinTuple TX)
{
	sercom = scm->bind<SERCOM_USART>(this);
	pinMode(RX, RX.iomux());
	pinMode(TX, TX.iomux());
}


void HardwareSerial::begin(uint32_t baudrate, SercomRXPad padRX, SercomUartTXPad padTX, uint16_t config)
{
